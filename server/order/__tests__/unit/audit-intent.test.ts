...
describe("createAuditIntentWriter", () => {
  ...
  it("supports the audit_pricing_decisions FK after a write (chain integrity)", async () => {
    const db: TestDb = await createTestDb();
    try {
      const writer = createAuditIntentWriter(db.db, log);
      await writer(
        buildIntentRow({
          intent_id: "01HW_INTENT_FK_OK",
          signal_ids: ["sig-1"],
          portfolio: "main",
          symbol: "OPT:SPY:2026-05-16:C:500",
          direction: "Short",
      ...
      // Should succeed now that the parent row exists.
      await db.run(
        `INSERT INTO audit_pricing_decisions VALUES
         (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
        [
          "01HW_DECISION",
          "01HW_INTENT_FK_OK",
          ...
          "cancel_if_crossed",
          "test",
          "2026-05-18T12:00:01.000Z",
        ],
      );

      const rows = await db.query(
        `SELECT decision_id FROM audit_pricing_decisions WHERE intent_id = ?`,
        ["01HW_INTENT_FK_OK"],
      );
      expect(rows).toHaveLength(1);
    } finally {
