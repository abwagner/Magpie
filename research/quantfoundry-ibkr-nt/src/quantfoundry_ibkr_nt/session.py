"""IBKR session abstraction (QF-240).
...
the QF observation bridge actually consumes from that adapter so the
bridge can be unit-tested without pulling NT in, and production wires
the real NT adapter behind the Protocol.

Mirrors ``quantfoundry-schwab-nt``'s split: data + Protocol here, the
NATS service in :mod:`quantfoundry_ibkr_nt.broker_bridge`. The IBKR
side is observation-only (per ``docs/tdd/broker-integration.md §2.3``);
no submit/cancel methods exist on this Protocol.
"""
...
class IbkrSessionClient(Protocol):
    ...
    tests pass a fake. The Protocol is observation-only — no
    submit/cancel/replace methods; QF doesn't submit IBKR orders.
    """
