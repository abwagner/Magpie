...
- ``SCHWAB_TOKEN_STORE_PATH`` (default ``/var/lib/quantfoundry/schwab-tokens.json``)
- Schwab auth secrets — same env contract as ``quantfoundry_schwab_nt.auth``:
  ``SCHWAB_APP_KEY``, ``SCHWAB_APP_SECRET``.

Run::

    uv run --package quantfoundry-md-bridge \\
        python -m quantfoundry_md_bridge.schwab
"""
...
async def amain() -> None:
    ...
    token_store_path = Path(
        os.environ.get(
            ...
            "/var/lib/quantfoundry/schwab-tokens.json",
        )
    )
