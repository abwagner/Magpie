"""``python -m quantfoundry_research serve`` — local dev runner.
...
.. code-block:: shell
    ...
    python -m quantfoundry_research serve

    # Bind a specific interface + port
    python -m quantfoundry_research serve --host 0.0.0.0 --port 8181

    # Reload-on-edit during development
    python -m quantfoundry_research serve --reload
"""
...
from quantfoundry_research.app import create_default_app
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.command == "serve":
        ...
        if args.reload:
            ...
            uvicorn.run(
                "quantfoundry_research.cli:_default_app_factory_for_reload",
                factory=True,
                host=args.host,
                port=args.port,
                reload=True,
