"""Tests for ``magpie_research.workers`` — stub pool + canned result."""

from __future__ import annotations

import pytest
from magpie_research.config import BacktestRunConfig
...
class TestSynchronousStubPool:
    ...
    async def test_dispatch_records_failure_when_inline_run_raises(
        ...
        monkeypatch.setattr("magpie_research.workers.stub_run", _boom)
        store = JobStore()
        pool = SynchronousStubPool()
        initial = await store.submit(config=_config())
        await pool.dispatch(
    ...
    async def test_emits_failed_status_when_pool_fails(
        ...
        monkeypatch.setattr(
            "magpie_research.workers.stub_run",
