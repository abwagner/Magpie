...
from quantfoundry_schwab_nt.order_status import derive_order_status
from quantfoundry_schwab_nt.streaming import Subscription as StreamerSubscription
...
def subjects_for(broker: str) -> dict[str, str]:
    ...
    return {
        "submit": f"orders.submit.{broker}",
        "cancel": f"orders.cancel.{broker}",
...
class SchwabBrokerBridge:
    ...
    asyncio event loop. Multi-process deployment requires running one
    bridge per process with disjoint NATS subjects (e.g., per-account
    suffixes), which is out of scope for QF-237's one-account-per-
    ...
    async def start(self) -> None:
        ...
        provided) the ACCT_ACTIVITY fan-out task."""
        if self._started:
            return
        self._subs.append(
    ...
    async def stop(self) -> None:
        ...
        self._started = False
        logger.info("SchwabBrokerBridge stopped", extra={"broker": self._broker})
