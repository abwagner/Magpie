...
class BrokerPosition:
    ...
    quantity: float
    # QF-272 — the raw Schwab account-snapshot position row, forwarded
    # verbatim so the TS /api/positions handler runs the same
    # categorization parser the REST fallback uses (identical rows from
    # either source; nothing is shaped Python-side). None for the thin
    # net-position view (reconciliation doesn't need it).
...
# ── AccountInfo (accounts reply) ──────────────────────────────────


@dataclass(frozen=True)
class AccountInfo:
    """Reply element of ``orders.accounts.<broker>`` (QF-272).

    Account-discovery metadata for the GUI's /api/accounts view. Mirrors
    the TS ``BrokerAccount`` shape (account number / hash / optional
    type). Discovery only — no balances.
    """
...
class BrokerExecReport:
    ...
    # QF-246 — M12-4: originating Schwab account id for multi-account
    # deploys. Omitted from the wire for the legacy "default" deploy so
    # the payload stays QF-237-compatible; set to the account id otherwise
    # so the TS audit-attribution path can read it.
