"""Tests for ``magpie_schwab_nt.exec_client``.
...
class TestListAccounts:
    @pytest.mark.asyncio
    async def test_lists_accounts_with_type_enrichment(self, tmp_path: Any) -> None:
        def handler(req: httpx.Request) -> httpx.Response:
            if req.url.path.endswith("/accounts/accountNumbers"):
                return httpx.Response(
                    200,
                    json=[
                        {"accountNumber": "111", "hashValue": "HASH111"},
                        {"accountNumber": "222", "hashValue": "HASH222"},
                    ],
                )
            if req.url.path.endswith("/accounts"):
                return httpx.Response(
                    ...
                    json=[
                        ...
                        {"securitiesAccount": {"accountNumber": "222", "type": "CASH"}},
                    ],
                )
        ...
        try:
            ...
            assert by_num["111"].hash_value == "HASH111"
            assert by_num["111"].account_type == "MARGIN"
            assert by_num["222"].account_type == "CASH"
        finally:
            await auth.aclose()

    @pytest.mark.asyncio
    async def test_type_enrichment_failure_is_best_effort(self, tmp_path: Any) -> None:
        def handler(req: httpx.Request) -> httpx.Response:
            if req.url.path.endswith("/accounts/accountNumbers"):
                return httpx.Response(
                    200, json=[{"accountNumber": "111", "hashValue": "HASH111"}]
                )
            # Bulk /accounts fails — type stays None, no raise.
            return httpx.Response(500, json={"error": "boom"})
        ...
        try:
            ...
            assert accounts[0].hash_value == "HASH111"
            assert accounts[0].account_type is None
        finally:
            await auth.aclose()

    @pytest.mark.asyncio
    async def test_account_numbers_4xx_raises(self, tmp_path: Any) -> None:
        def handler(req: httpx.Request) -> httpx.Response:
