...
| [deployment-topology.md](tdd/deployment-topology.md)                   | Where the QF TS server + its infra run (server vs laptop split); single-writer / MinIO IAM rationale; migration sequence unblocking QF-201 + M10-6.                           |
...
| [research-orchestrator.md](tdd/research-orchestrator.md) | Python backtest-job orchestrator (`research/magpie-research/`) — job lifecycle, NATS event surface (`research.jobs.*` + `data.write.results`), stub worker pool, single-writer guard.                                          |
| [supervisor.md](tdd/supervisor.md)                       | What scheduling/supervision actually exists in Architecture B — the ingest scheduler (`scripts/scheduler.ts` container + `deploy/systemd/` timers) and per-broker NT-bundle supervision. The retired signal-tick supervisor is gone. |
...
| [../data/CRON.md](../data/CRON.md)  | Live cron schedule (the `magpie-scheduler` container) — job names, schedules, ops.                                                                |
